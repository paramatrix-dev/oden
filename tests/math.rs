...
fn test_addition() {
    ...
    assert_eq!(eval_str("1mm + 2mm"), Ok(Value::Length(3.mm())));
}

#[test]
fn test_subtraction() {
    assert_eq!(eval_str("2 - 1"), Ok(Value::Number(1.)));
    assert_eq!(eval_str("2mm - 1mm"), Ok(Value::Length(1.mm())));
}

#[test]
fn test_multiplication() {
    assert_eq!(eval_str("2 * 3"), Ok(Value::Number(6.)));
    assert_eq!(eval_str("2mm * 3"), Ok(Value::Length(6.mm())));
    assert_eq!(eval_str("2 * 3mm"), Ok(Value::Length(6.mm())));
}

#[test]
fn test_division() {
    assert_eq!(eval_str("6 / 2"), Ok(Value::Number(3.)));
    assert_eq!(eval_str("6mm / 2"), Ok(Value::Length(3.mm())));
}
...
fn test_negative_number() {
    ...
    assert_eq!(eval_str("-2mm"), Ok(Value::Length(-2.mm())));
}
